...
const LoadingScreen = ({ isDataLoading, onFinish }) => {
  ...
  const progressRef = useRef(0);            // tracks the real value without re-render delays
  const dataReadyRef = useRef(false);       // latches when isDataLoading becomes false
  const finishedRef = useRef(false);        // prevents calling onFinish more than once

  const stableOnFinish = useCallback(onFinish, [onFinish]);

  // Latch the "data ready" flag so the interval loop can see it
  useEffect(() => {
    if (!isDataLoading) {
  ...
  // Single interval drives the whole animation:
  //   Phase 1 — simulate 0 → 90  (while data is still loading)
  //   Phase 2 — accelerate 90 → 100  (once data is ready)
  //   Then call onFinish and stop.
  useEffect(() => {
    const INTERVAL = 30; // ms between ticks

    const timer = setInterval(() => {
      const current = progressRef.current;

      if (!dataReadyRef.current) {
        // ─── Phase 1: crawl toward 90 ───
        if (current < 90) {
          const remaining = 90 - current;
          const step = Math.max(0.15, remaining * 0.03);
          const next = Math.min(current + step, 90);
        ...
        // ─── Phase 2: rush to 100 ───
        if (current < 100) {
          const next = Math.min(current + 2.5, 100);
          progressRef.current = next;
          setProgress(next);
        } else if (!finishedRef.current) {
          // We've hit 100 — wait a beat then dismiss
      ...
      <div className="w-full max-w-xl px-6 flex flex-col items-center">
        ...
        <div className="relative w-full h-20 mb-6 px-4">
          ...
          <div
            ...
            style={{
              width: `${progress}%`,
              transitionDuration: progress >= 99 ? '500ms' : '80ms'
            }}
          />
